...
import support_files_tractor as sfc
...
fig_x=16
fig_y=16
fig=plt.figure(figsize=(fig_x,fig_y),dpi=120,facecolor=(0.5,0.5,0.8))
n=8 
m=5
...
ax0=fig.add_subplot(gs[0:3,:3],facecolor=(0.5,0.5,0.8))

# Plot the reference trajectory
ref_trajectory=ax0.plot(X_ref,Y_ref,'k',linewidth=1)

# Plot the lanes
lane_width=constants[16]
lane_1,=ax0.plot([X_ref[0],X_ref[frame_amount]],[lane_width/5,lane_width/5],'k',linewidth=0.2)
lane_2,=ax0.plot([X_ref[0],X_ref[frame_amount]],[-lane_width/5,-lane_width/5],'k',linewidth=0.2)
...
lane_5,=ax0.plot([X_ref[0],X_ref[frame_amount]],[lane_width/2+2*lane_width,lane_width/2+2*lane_width],'k',linewidth=0.2)
lane_6,=ax0.plot([X_ref[0],X_ref[frame_amount]],[-lane_width/2-2*lane_width,-lane_width/2-2*lane_width],'k',linewidth=0.2)

# Draw a motorcycle
car_1,=ax0.plot([],[],'k',linewidth=3)
car_predicted,=ax0.plot([],[],'-g',linewidth=1)
car_determined,=ax0.plot([],[],'y',linewidth=1)
...
plt.xlabel('longitudinal distance [m]',fontsize=12)

# Create an object for the motorcycle (zoomed)
ax1=fig.add_subplot(gs[4:8,:3],facecolor=(0.5,0.5,0.8))
bbox_props_angle=dict(boxstyle='square',fc=(0.5,0.5,0.8),ec='k',lw='1')
bbox_props_steer_angle=dict(boxstyle='square',fc=(0.5,0.5,0.8),ec='g',lw='1')

neutral_line=ax1.plot([-100,100],[0,0],'y',linewidth=1)
car_1_body,=ax1.plot([],[],'k',linewidth=3)
car_1_body_extension,=ax1.plot([],[],'--k',linewidth=0.5)
car_1_back_wheel,=ax1.plot([],[],'b',linewidth=4)
car_1_front_wheel,=ax1.plot([],[],'b',linewidth=4)
car_1_front_wheel_extension,=ax1.plot([],[],'--g',linewidth=1)
...
plt.xlabel('longitudinal distance [m]',fontsize=12)
plt.ylabel('lateral distance [m]',fontsize=12)
yaw_angle_text=ax1.text(25,2,'',size='10',color='k',bbox=bbox_props_angle)
steer_angle=ax1.text(25,-2,'',size='10',color='g',bbox=bbox_props_steer_angle)

# Create the function for the steering wheel
ax2=fig.add_subplot(gs[6:8,3:],facecolor=(0.5,0.5,0.8))
steering_wheel,=ax2.plot([],[],'-g',linewidth=2,label='steering angle [rad]')
plt.xlim(0,t[-1])
...
ax3=fig.add_subplot(gs[0:2,3:],facecolor=(0.5,0.5,0.8))
yaw_angle_reference=ax3.plot(t,psi_ref,'-k',linewidth=2,label='yaw reference [rad]')
yaw_angle,=ax3.plot([],[],'-y',linewidth=2,label='yaw angle [rad]')
if PID_switch!=1:
    psi_predicted,=ax3.plot([],[],'-g',linewidth=3,label=' predicted yaw angle [rad]')
plt.xlim(0,t[-1])
...
ax4=fig.add_subplot(gs[3:5,3:],facecolor=(0.5,0.5,0.8))
Y_position_reference=ax4.plot(t,Y_ref,'-k',linewidth=2,label='Y - reference [m]')
Y_position,=ax4.plot([],[],'-y',linewidth=2,label='Y - position [m]')
if PID_switch!=1:
    Y_predicted,=ax4.plot([],[],'-g',linewidth=3,label='Y - predicted [m]')
plt.xlim(0,t[-1])
...
plt.plot(X_ref,statesTotal[:,3],'--r',linewidth=2,label='Tractor position')
...
plt.plot(t,statesTotal[:,3],'--r',linewidth=2,label='Tractor Y position')
plt.xlabel('t-time [s]',fontsize=15)
plt.ylabel('y-position [m]',fontsize=15)
plt.grid(True)
plt.legend(loc='center right',fontsize='small')
